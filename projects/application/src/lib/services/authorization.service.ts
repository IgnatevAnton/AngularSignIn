...
export class AuthorizationService implements IAuthorizeService {
  ...
  private _title: string = "_AuthorizationService";
  ...
  get isLoadingVerificationUser$(): Signal<boolean> { return this._isLoadingVerificationUser$.asReadonly(); }
  get isErrorVerificationUser$(): Signal<boolean> { return this._isErrorVerificationUser$.asReadonly(); }

  get isTimeoutRepeatSendCode$(): Signal<number> { return this._isTimeoutRepeatSendCode$.asReadonly(); }


  @DomainDecoators.DebugMethod()
  check(): void {
    this._repository.check().pipe(take(1)).subscribe({
      next: (user) => {
  ...
  login(username: string, password: string): void {
    ...
    this._repository.login(username, password).pipe(take(1)).subscribe({
      next: (user) => {
  ...
  registration(data: DomainInterface.IUserRegistration): void {
    ...
    this._repository.registration(data).pipe(take(1)).subscribe({
      next: (status: DomainInterface.IUserRegistrationStatus) => {
        ...
        if (status.status) {
          ...
          setTimeout(() => { this._isErrorRegistration$.set([]); }, this._timeoutMillisecondCleanError);
        }
      },
      error: (error) => {
        this._isLoadingRegistration$.set(false);
        this._isErrorRegistration$.set([RegistrationStatusErrors.EMAIL, RegistrationStatusErrors.PASSWORD, RegistrationStatusErrors.USER_NAME]);
        setTimeout(() => { this._isErrorRegistration$.set([]); }, this._timeoutMillisecondCleanError);
        this._logger?.warning(this._title, error);
      }
    });
  ...
  confirm(code: string): void {
    ...
    this._repository.confirm(code).pipe(take(1)).subscribe({
      next: (status: boolean) => {
        ...
        if (status) {
          ...
          setTimeout(() => { this._isErrorVerificationUser$.set(false); }, this._timeoutMillisecondCleanError);
        }
      },
      error: (error) => {
        this._logger?.warning(this._title, error);
  ...
  sendCode(): void {
    ...
    interval(1000).pipe(take(this._timeoutSecondRepeatCode + 1)).subscribe(time => {
